...
class FeatEncode(nn.Module):
    ...
    def __init__(self, time_dims, feat_dims, out_dims):
        ...
        self.feat_encoder = nn.Linear(time_dims + feat_dims + 1, out_dims)
        self.reset_parameters()
    ...
    def forward(self, edge_feats, edge_ts, edge_w):
        edge_time_feats = self.time_encoder(edge_ts)
        x = torch.cat([edge_feats, edge_w, edge_time_feats], dim=1)
...
class MLPMixer(nn.Module):
    ...
    def forward(self, edge_feats, edge_ts, edge_w, batch_size, inds):
        # x :     [ batch_size, graph_size, edge_dims+time_dims]
        edge_time_feats = self.feat_encoder(edge_feats, edge_ts, edge_w)
