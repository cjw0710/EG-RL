...
def name_fn(args, mixer_configs):
    rl_tag = "withRL" if args.rl_on else "noRL"
    fn = 'results/%s_%s_neighbors%d_edges%d_layers%d_%dhop' % (
        args.data, rl_tag, args.num_neighbors, args.max_edges, args.num_layers, args.sampled_num_hops
    )
...
def get_args():
    ...
    parser.add_argument('--grad_file', type=str, default='data/edge_list_grad.npy')
    ...
    parser.add_argument('--rl_on', action='store_true', help='Enable RL filtering')
    parser.add_argument('--max_k', type=int, default=512, help='Max candidate edges for RL filtering')
...
def load_all_data(args):
    ...
    if node_feats is not None:
        node_feats = node_feats.to(args.device)  # move only when exists
...
if __name__ == "__main__":
    ...
    if args.use_grad:
        import numpy as np
        grad_array = np.load(args.grad_file, allow_pickle=False)
        edge_grad_stream.set_grad_data(grad_array)
